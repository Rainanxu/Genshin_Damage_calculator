...
*/
int autoputin_new(int char_in[], int number_now, int char_from[], int number_ask_for, int finded[])

{
	...
	if (find_from == 0) {
		...
		for (char* char_in_times = 0;;) {
			//char* char_in_times = 0;
			char_in_times = &char_in[number_now];
			ch0 = scanf_s("%c", char_in_times, 1);
	...
	i0 = 0;
	for (int il0 = 0,il1=0;;) {
		if (char_from[il0] == 0)break;
		if (finded[il1 + 1] = if_like(char_in, char_from[il0], 4, 2,0)) {
			finded[il1] = f = il0 + 1;
			il1++, il1++, times++;
		}
		il0++;
