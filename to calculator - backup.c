...
{
    //输入判定
    int chis, gettims = 1, chok = 0, chto1 = 0, chexit = 0, chidhelp = 0, chid1 = 0, chabout = 0, chmessage = 0, chtoc1 = 0, chin1 = 0, chin2 = 0, chin[13] = { 0 };
    //字母判定
    char ok = 'A', exit = 'A', idhelp = 'A', about = 'A', message = 'A', chto = 'A', toc1 = 'A';
    //假意接收
    char hole1 = 'A';
    int hole2 = 0;

    //功能部件
    int ch0, ch1, getcharb, toc2 = 0, toc3 = 3;
    char id = 'A', is = 'A';
    ...
    //伤害乘区
    float A1 = 1.0, M1 = 1.0, B1 = 1.0, C1 = 1.0, E1 = 1.0, D1 = 1.0, R1 = 1.0;
...
reputin:
    ...
    {
        //—————————————————part 1————————————————————//
        while (chok == 0 || chexit == 0 || chidhelp == 0 || chabout == 0 || chmessage == 0 || chtoc1 == 0 || chin1 == 0)//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
        {
            ...
            if (chto == 'O')
            {
                ...
                chtoc1 = scanf_s("POIN%c", &toc1, 1);
                if (toc1 != 'T') chtoc1 = 0;
                if (chtoc1 == 0) {
                    chtoc1 = scanf_s("DAMAG%c", &toc1, 1);
                    if (toc1 != 'E') chtoc1 = 0;
                }
            ...
            if (chok != 0 || chexit != 0 || chidhelp != 0 || chabout != 0 || chmessage != 0 || chtoc1 != 0|| chin1 != 0)break;//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
            if (gettims >= 2) {
                printf("\r");
                gettims--;
            }
            if (chok == 0 || chexit == 0 || chidhelp == 0 || chabout == 0 || chmessage == 0 || chtoc1 == 0|| chin1 == 0) {//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
                printf("输入失败，请重试：");
                getcharb = getchar();
                gettims++;
            }
        ...
        for (;;)
        {
            ...
            if (toc1 == 'E' && toc2 == 0 || toc1 == 'T' && toc2 == 1) break;
            ...
            if (chto1 == 0) {chexit = 0, chidhelp = 0, chid1 = 0, chabout = 0, chmessage = 0, chtoc1 = 0;}//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
            if (chto == 'O')
            {
                chidhelp = scanf_s("IDHEL%c", &idhelp, 1);
                if (idhelp != 'P') chidhelp = 0;
    ...
    if (toc1 == 'T' && toc2 == 1) { toc1 = 'A'; toc2 = 0; system("cls"); goto reputin; }
    if (toc1 == 'E' && toc2 == 0) { toc1 = 'A'; toc2 = 1; system("cls"); goto reputin; }
    printf("\r");
    printf("数据输入完成！    \n\n");
    printf("现有信息如下：\n\n  基础属性\n    生命值上限 Max HP                     %.0f\n    攻击力 ATK                            %.0f\n    防御力 DEF                            %.0f\n    元素精通 Elemental Mastery            %.0f\n    体力上限 Max Stamina                  %.0f\n\n  进阶属性\n    暴击率 CRIT Rate                      %.0f%%\n    暴击伤害 CRIT DMG                     %.0f%%\n    治疗加成 Healing Bonus                %.0f%%\n    受治疗加成 Incoming Healing Bonus     %.0f%%\n    元素充能效率 Energy Recharge          %.0f%%\n    冷却缩减 CD Reduction                 %.0f%%\n    护盾强效 Shield Strength              %.0f%%\n    伤害加成 DMG Bonus                    %.0f%%\n\n请继续：", hp1, atk1, def1, em1, ms1, cr1 * 100, cd1 * 100, hb1 * 100, ihb1 * 100, er1 * 100, cdr1 * 100, ss1 * 100, db1 * 100);
